...
\author{Pratham Gupta Sehaj Ganjoo\\
        Krishna Aggarwal Gavish Bansal}
...
\begin{frame}{Formal Definition of $\lambda$-Terms}
  ...
  \begin{itemize}
      ...
      \item let $N = y $
      \item *fix this* $(M N) = (\lambda x.x \space \space  y) \rightarrow y$
  \end{itemize}
\end{frame}
...
\begin{frame}{Notation Conventions}
  \begin{itemize}
    \item Left-association of application:
      \[
        (((FM_1)M_2)\ldots M_n) \quad\text{is written as}\quad F M_1 \ldots M_n.
      \]
...
% Section 3: Free and Bound Variables
...
\begin{frame}{Free vs. Bound Variables}
  ...
  \vspace{1em}
  \[
  \text{For } M = \lambda x. (xy), \quad \text{FV}(M)=\{y\},\quad \text{BV}(M)=\{x\}.
  \]
\end{frame}

\begin{frame}{Definition of FV and BV}
  \begin{block}{Inductive Definitions}
    \begin{enumerate}
      \item If \( M = x \), then \(\text{FV}(x)=\{x\}\) and \(\text{BV}(x)=\emptyset\).
      \item If \( M = (M_1 M_2) \), then:
        \[
        \text{FV}(M)=\text{FV}(M_1)\cup \text{FV}(M_2),
        \]
        \[
        \text{BV}(M)=\text{BV}(M_1)\cup \text{BV}(M_2).
        \]
